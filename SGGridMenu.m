...
- (void)layoutSubviews
{
    ...
    self.titleLabel.frame = (CGRect){CGPointZero, CGSizeMake(self.bounds.size.width, 37)};
    ...
    self.cancelButton.frame = CGRectMake(0, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height, self.bounds.size.width, 42);
...
{
    UIEdgeInsets margin = UIEdgeInsetsMake(0, 33, 15, 33);
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 3, self.bounds.size.width/3.7);
