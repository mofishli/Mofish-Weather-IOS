...
- (void)buildView {
    ...
    CGRect h=CGRectMake(self.downCenterRect.origin.x, self.downCenterRect.origin.y+3, self.downCenterRect.size.width, self.downCenterRect.size.height);
    
    self.downView                     = [[UIView alloc] initWithFrame:h];
    self.downView.layer.masksToBounds = YES;
    [self addSubview:self.downView];
    
    // 创建出太阳的view并存储一些相关参数
    self.sunImageView           = [[UIImageView alloc] initWithFrame:self.upView.frame];
    self.sunImageView.image     = [UIImage imageNamed:@"weather-clear"];
    ...
    self.moonImageView.image     = [UIImage imageNamed:@"weather-moon"];
    ...
    // 中间的线条
    self.lineView                 = [[UIView alloc] initWithFrame:CGRectMake(0, self.height / 2.f+3, self.width, 2)];
